...
const SYSTEM_PROMPT = `[ROLE] ISO 9001:2015 QMS implementation consultant. [SOURCE OF TRUTH] Use ONLY the structured organisation context provided. [DETERMINISM] If industry or products are missing, return exactly the string: INSUFFICIENT_DATA [OUTPUT RULES] Respond with a valid JSON array ONLY. Start your response with [ and end with ]. No markdown fences. No preamble. No explanation. No text before or after the JSON. Each object in the array must have exactly these keys: {"doc_ref":"sequential e.g. QP-001 procedures QF-001 forms QR-001 records","title":"document title","doc_type":"one of exactly: Procedure|Policy|Work Instruction|Form|Record|Other","version":"1.0 DRAFT","owner":"role title only","review_date":"YYYY-MM-DD date 12 months from today","status":"Draft"} Generate exactly 8 documents: the most critical mandatory ISO 9001:2015 documented information for the described industry. Keep each object small. [FABRICATION GUARD] Document titles and structure only. No document content. No invented regulatory document titles unless the user described those regulations.`
...
export default function DocumentRegister() {
  ...
  const { priorContext, priorLoading, orgProfile } = useQMSContext('ISO 9001:2015 Cl.7.5', activeProgramme?.id)
  const nextModule = NEXT_MODULE['ISO 9001:2015 Cl.7.5']
  ...
  return (
    <div className="max-w-4xl">
      ...
      {canEdit && <QMSAIGenerator clause="ISO 9001:2015 Cl.7.5" systemPrompt={SYSTEM_PROMPT} requiredFields={REQUIRED} placeholder="Describe your organisation — industry, size, products/services, any existing documents..." onGenerated={onGenerated} priorContext={priorContext} orgProfile={orgProfile} />}
      {showForm && (
